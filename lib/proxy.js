...
function proxyOnConnect(ghost) {
  return function proxyOnConnect_(req, client, header) {
    ...
    const reqUrl = 'https://'+ (port === '443' ? host : req.url);

    if ( ! (host in ghost._sessions)) {
      const cert = snakeoil.generateCert(host);
      ghost.addContext(host, cert);

      const agent = new https.Agent({ keepAlive: true });

      ghost._sessions[host] = { cert, agent };
    }
