...
function request(req, res) {
  ...
  const tReq = _request(reqUrl, tRes => {
    console.log('-----');
    console.log(tRes.headers);
    setHeaders(res, tRes.rawHeaders, true);
...
function setHeaders(msg, headers, d) {
  for (let i = 0, l = headers.length; i < l; i += 2) {
    if (d) {
      console.log(headers[i], headers[i + 1]);
    }
    msg.setHeader(headers[i], headers[i + 1]);
  }
}
